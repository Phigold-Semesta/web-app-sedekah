...
@section('title', 'Ubah Profil Data Donatur')
...
<div class="min-h-screen bg-slate-50/50 flex items-center justify-center p-4 antialiased">
    <div class="w-full max-w-2xl bg-white rounded-[2rem] shadow-2xl shadow-slate-200/80 border border-slate-100 p-8 sm:p-10 relative overflow-hidden">
        ...
        <div class="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-teal-600 via-emerald-600 to-emerald-800"></div>
        ...
        <div class="mb-8 text-center sm:text-left flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-6">
            <div>
                <h1 class="text-2xl sm:text-3xl font-black text-emerald-950 tracking-tight uppercase italic">
                    Modifikasi <span class="text-emerald-600 font-extrabold not-italic">Data Donatur</span>
                </h1>
                <p class="text-xs sm:text-sm font-medium text-slate-400 mt-1 uppercase tracking-widest">
                    SEDEKAH • DATA MODIFICATION PROTOCOL
                </p>
            </div>
            <div>
                <span class="inline-block px-4 py-1.5 bg-amber-50 text-amber-700 text-[10px] font-black tracking-widest uppercase rounded-full border border-amber-200/50 shadow-sm animate-pulse">
                    Editing Mode
                </span>
            </div>
        </div>
        ...
        @if ($errors->any())
            <div class="mb-6 p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center gap-3 text-rose-600">
                ...
                <span class="text-xs sm:text-sm font-bold tracking-wide">Pembaruan ditolak! Silakan periksa kembali berkas kolom database Anda.</span>
            </div>
        @endif

        <form action="{{ route('admin.donatur.update', $donatur->id_donatur) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <div class="bg-slate-50 rounded-2xl p-4 flex items-center justify-between border border-slate-100/80">
                <span class="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    ...
                    </svg> Target Primary Key
                </span>
                <span class="text-xs font-black bg-white px-3 py-1 rounded-lg text-slate-500 font-mono shadow-sm border border-slate-200/50">
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <input type="text" name="nama_donatur" id="nama_donatur" 
                        class="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-full text-sm font-bold text-slate-700 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 focus:bg-white transition-all duration-300"
                        value="{{ old('nama_donatur', $donatur->nama_donatur) }}" required>
                </div>
                @error('nama_donatur')
                    <p class="text-xs font-bold text-rose-500 pl-2 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <input type="text" name="no_hp" id="no_hp" 
                        class="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-full text-sm font-bold text-slate-700 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 focus:bg-white transition-all duration-300"
                        value="{{ old('no_hp', $donatur->no_hp) }}" required>
                </div>
                @error('no_hp')
                    <p class="text-xs font-bold text-rose-500 pl-2 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <textarea name="alamat" id="alamat" rows="3" 
                        class="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-[1.5rem] text-sm font-bold text-slate-700 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 focus:bg-white transition-all duration-300 resize-none" required>{{ old('alamat', $donatur->alamat) }}</textarea>
                </div>
                @error('alamat')
                    <p class="text-xs font-bold text-rose-500 pl-2 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="pt-4 flex flex-col sm:flex-row items-center justify-end gap-3 border-t border-slate-100 mt-6">
