    ...
    <div class="bg-white dark:bg-slate-900 rounded-[3.5rem] border border-emerald-50 dark:border-emerald-900/20 shadow-2xl overflow-hidden relative">
        ...
        <div class="p-8 md:p-14 relative z-10">
            ...
            <div class="flex flex-col md:flex-row items-center gap-10 mb-14 pb-14 border-b border-slate-100 dark:border-slate-800/50">
                <div class="relative">
                    <div class="w-36 h-36 bg-gradient-to-tr from-[#008f5d] to-emerald-400 rounded-[3rem] rotate-6 flex items-center justify-center shadow-2xl p-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="group space-y-3">
                    ...
                    <div class="flex items-center gap-5 p-6 bg-slate-50/50 dark:bg-slate-800/40 rounded-[2rem] border border-transparent hover:border-emerald-100 dark:hover:border-emerald-900/30 transition-all">
            ...
            <div class="mt-14 pt-10 border-t border-slate-100 dark:border-slate-800 flex flex-col md:flex-row gap-5">
                ...
                <form id="delete-form-{{ $user->id_user }}" action="{{ route('direktur.manajemen_user.destroy', $user->id_user) }}" method="POST" class="flex-1">
                    @csrf
                    @method('DELETE')
                    <button type="button" onclick="confirmDelete('{{ $user->id_user }}', '{{ $user->nama_user }}')" 
                        ...
                        <i class="fas fa-archive mr-2"></i> Archive Identity
                    </button>
                </form>
                @else
                ...
                <a href="{{ route('direktur.manajemen_user.edit', $user->id_user) }}" 
                    class="flex-[2] bg-[#008f5d] text-white py-5 rounded-[2rem] text-[11px] font-black uppercase tracking-[0.2em] text-center hover:bg-emerald-700 hover:shadow-2xl transition-all active:scale-[0.98] shadow-lg flex items-center justify-center gap-3">
    ...
    function confirmDelete(id, name) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    title: 'Synchronizing...',
                    html: '<p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Updating database registry</p>',
                    background: isDark ? '#0f172a' : '#ffffff',
                    color: isDark ? '#f1f5f9' : '#1e293b',
