...
<div class="p-4 md:p-10 bg-slate-50 dark:bg-slate-950 min-h-screen flex justify-center items-center animate__animated animate__fadeIn">
    <div class="w-full max-w-2xl bg-white dark:bg-slate-900 p-8 md:p-16 rounded-[3rem] md:rounded-[5rem] shadow-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <form action="{{ route('admin.manajemen_user.store') }}" method="POST" class="space-y-8" autocomplete="off">
                @csrf
                ...
                <div class="group space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="nama_user" value="{{ old('nama_user') }}" required 
                            class="w-full pl-14 pr-6 py-5 bg-slate-50 dark:bg-slate-800/50 border-2 @error('nama_user') border-red-500 @else border-transparent @enderror rounded-3xl text-sm font-bold text-slate-700 dark:text-white custom-focus transition-all uppercase placeholder:normal-case"
                            placeholder="John Doe">
                    </div>
                    @error('nama_user')
                        <p class="text-xs font-semibold text-red-500 ml-2 animate__animated animate__headShake">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="group space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="username" value="{{ old('username') }}" required 
                                class="w-full pl-14 pr-6 py-5 bg-slate-50 dark:bg-slate-800/50 border-2 @error('username') border-red-500 @else border-transparent @enderror rounded-3xl text-sm font-bold text-slate-700 dark:text-white custom-focus transition-all"
                    ...
                    <div class="group space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="password" name="password" id="password" required 
                                class="w-full pl-14 pr-14 py-5 bg-slate-50 dark:bg-slate-800/50 border-2 @error('password') border-red-500 @else border-transparent @enderror rounded-3xl text-sm font-bold text-slate-700 dark:text-white custom-focus transition-all"
                                placeholder="••••••••">
