...
@section('content')
<div class="w-full max-w-2xl mx-auto px-4 py-8 md:py-12 lg:py-16">
        ...
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="nama_barang" 
                       class="w-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-2xl md:rounded-3xl px-5 py-4 md:py-5 focus:border-amber-500 outline-none text-base md:text-lg text-slate-800 dark:text-white font-bold transition-all" 
            ...
            <div>
                <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-2 ml-2">Metode Pengiriman</label>
                <select name="jasa_pengiriman" 
                        class="w-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-2xl md:rounded-3xl px-5 py-4 md:py-5 focus:border-amber-500 outline-none text-base md:text-lg text-slate-800 dark:text-white font-bold transition-all appearance-none cursor-pointer" required>
                    <option value="" disabled selected>Pilih Metode Pengiriman</option>
                    <option value="pickup">Tim Yayasan Menjemput</option>
                    <option value="kurir_pribadi">Diantar Sendiri (Kurir Pribadi)</option>
                    <option value="ekspedisi">Jasa Ekspedisi (JNE/J&T/dll)</option>
                </select>
            </div>
        </div>

        <button type="submit" 
                class="w-full mt-10 py-4 md:py-5 bg-amber-600 hover:bg-amber-700 text-white rounded-2xl md:rounded-3xl font-black uppercase tracking-widest text-xs transition-all shadow-lg shadow-amber-700/20 active:scale-[0.98]">
            Ajukan Donasi Barang
        </button>
        
        <p class="text-[10px] text-center mt-6 text-slate-400 uppercase tracking-widest">
            <i class="fas fa-truck mr-1"></i> Data akan diverifikasi sebelum penjemputan
        </p>
    </form>
</div>
@endsection