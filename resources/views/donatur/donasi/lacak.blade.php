...
@section('page_title', 'Detail Lacak Donasi')

@section('content')
<div class="max-w-5xl mx-auto px-4 py-8">
    {{-- Back Button --}}
    <div class="mb-8">
        <a href="{{ route('donatur.lacak.index') }}" 
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none">
                <h2 class="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter mb-4">Peta Lokasi Terkini</h2>
                <div id="map" class="w-full h-96 rounded-3xl z-10"></div>
            </div>
        </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none h-full">
                <h2 class="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter mb-6">Riwayat</h2>
        ...
        if (tracks.length > 0) {
            ...
            tracks.forEach(track => {
                L.marker([track.latitude, track.longitude])
                    .addTo(map)
                    .bindPopup(`<b>${track.status_pelacakan}</b><br>${track.created_at}`);
            });
        } else {
            // Jika belum ada data, tampilkan pesan di dalam map
            document.getElementById('map').innerHTML = '<div class="flex items-center justify-center h-full text-slate-400 font-bold uppercase tracking-widest">Data Peta Belum Tersedia</div>';
        }
    });
</script>
@endsection